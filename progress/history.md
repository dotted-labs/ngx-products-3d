...
## 2026-07-13 — spec-03 Fase 4 — feature 6 `badge-texture-scene` → done (review aprobada a la primera; N3 diferido a features 7/9)

- **Rol**: leader (orquestación); 1 implementer → 1 reviewer (**APROBADO** sin bloqueantes, `progress/review_feature6.md`).
- **Implementer** (`progress/impl_feature6.md`): implementado el stub `badge-texture.component.ts` (escena secundaria del frente, se renderizará a textura vía NgtsRenderTexture en la feature 7; no crea canvas propio). Contenido: cámara propia, plano de fondo con textura base por tier (gate `hasValue()` vía `resourceValueOrUndefined`, colorSpace sRGB mutado en effect post-resolución, warn dev en error) y 3 × `ngts-text-3d` (name / #memberNumber / TIER) con `theme().fontUrl` y color `theme.colors?.text ?? BADGE_TEXT.color`. Nuevo `badge-texture.ts` con fns puras testeadas: `resolveBaseTextureUrl` (fallback de tier), `badgeTextFor` (formato por slot), `fitTextScale` (clamp ≤1, ancho no medible → 1). Config: `BADGE_TEXTURE` += `cameraPosition`/`planeSize`; nuevas `BADGE_TEXT` (prefijo `#`, `maxWidth`, color fallback) y `BADGE_TEXT_LAYOUT` (`BadgeTextSlot[]` data-driven, iterada con `@for` — reordenar slots no toca el componente).
- **Decisiones clave (API real gana, verificadas en node_modules por implementer Y reviewer)**:
  1. **Cámara**: `<ngts-perspective-camera>` de soba con `options.makeDefault` — el `<ngt-perspective-camera>` crudo del brief NO tiene concepto de `makeDefault` en angular-three v4 (0 ocurrencias en fesm2022). Registra la cámara como default del store del portal del RenderTexture.
  2. **Nombre largo**: bbox manual (`NgtsResize` no existe en soba v4, spike S3). Effect reactivo a `getInstanceState(mesh)?.nonObjects()` (API pública de angular-three; se bumpea al attachear/recrear la TextGeometry cuando resuelve la fuente o cambia member) → `computeBoundingBox()` + `mesh.scale.setScalar(fitTextScale(...))`. Se descartó `ngts-center`: su effect no trackea el attach profundo de la geometría (mediría la geometría vacía y no re-mediría). Cero trabajo por frame (sin beforeRender).
  3. **Textos unlit** (`ngt-mesh-basic-material`): la escena del RenderTexture no tiene luces; un material lit pintaría negro.
- **Review**: perímetro exacto, CA1–CA7 verificados, N2 íntegro re-ejecutado por el reviewer (build ✅ · lint ✅ · test **68/68** (9 nuevos) ✅ · playground ✅ · dist sin deps fantasma). 3 hallazgos menores no bloqueantes: import >100 chars (lint pasa → se deja, Prettier no está instalado), comentario con nombre de effect inexistente (**corregido en el cierre**), CRLF preexistente de `badge.config.ts` (no tocado).
- **Condición de cierre del reviewer**: la **checklist N3 de Fase 4** (member reactivo, tier desconocido → textura default visible, encaje del nombre largo en pantalla, sRGB) es **OBLIGATORIA en la review de la feature 7/9** — hoy no existe superficie visual montada (este componente lo monta la feature 7 dentro del RenderTexture); la Fase 4 no puede cerrarse sin ella.
- **Verificación final del leader (post-cierre, tras corregir el comentario)**: build ✅ · lint ✅ · test 68/68 ✅.
- Feature 6 → `done`.
- **Siguiente**: Fase 4 — feature 7 `badge-rendertexture-map` (depende de 3 ✅ y 6 ✅). Recordar: aplicar `BADGE_MAP_ANISOTROPY` a la textura real (seguimiento de f3) + checklist N3 de Fase 4 + arreglar de paso el import >100 chars si Prettier entra en juego.

## 2026-07-13/14 — spec-03 Fase 4 — feature 7 `badge-rendertexture-map` → done (review con condición N3 → fix playground → N3 ✅)

- **Rol**: leader (orquestación); 1 implementer → 1 reviewer (**APROBADA CON CONDICIONES**, `progress/review_feature7.md`) → 1 fix post-N3 (`progress/impl_feature7_fix1.md`) → N3 del usuario ✅.
- **Implementer** (`progress/impl_feature7.md`): `NgtsRenderTexture` (tupla soba) montado como `map` del `<ngt-mesh-physical-material>` de la tarjeta en `badge-scene.component.ts`: `attach="map"` + `<ng-template renderTextureContent>` envolviendo `<products-3d-badge-texture [member] [theme]>`. `renderTextureOptions` 100% config-driven (`width/height = BADGE_TEXTURE.size`, `frames`, `anisotropy`).
- **Decisiones clave (API real gana, verificadas en node_modules por implementer Y reviewer)**:
  1. **`frames: Infinity` (continuo) en vez del `frames: 1` preferente de la spec**: el contador del `NgtsRenderTextureContainer` solo se resetea al re-ejecutarse su effect (trackea renderPriority + store del portal); el montaje async del contenido y los cambios de member/theme NO lo resetean → `frames: 1` = frente en blanco congelado y sin reactividad; no hay API pública de invalidación. Coste por-frame documentado en JSDoc de `BADGE_TEXTURE.frames` (ruta permitida por la acceptance).
  2. **Anisotropy a las options del RenderTexture**: el binding `[mapAnisotropy]` del material (feature 3) era no-op en angular-three v4 (pierce solo claves con punto); soba aplica las options restantes como `parameters` sobre `fbo.texture` → `fbo.texture.anisotropy = 16` real. Seguimiento de f3 cerrado.
- **Reactividad**: member/theme (signals) leídos dentro del template del portal; con frames continuo cada cambio se pinta al frame siguiente sin recrear el canvas.
- **Review**: todo lo automatizable verde (build ✅ · lint ✅ · test **69/69** (1 nuevo) ✅ · playground ✅ · dist limpio); condición única = checklist N3 de Fase 4 en navegador (heredada de la review de f6).
- **N3 ronda 1 (usuario): FALLÓ 1-3** — frente de la tarjeta negro, correa blanca lisa (captura). **Causa raíz (fix1, verificada)**: URLs `.jpg` inexistentes en el signal `theme` de `badge-demo.component.ts` (los assets reales son `.png`); el input `[theme]` pisa por completo al provider de la ruta (`resolvedTheme` no hace merge) → 404 en las 4 texturas → gates no-lanzantes de la lib degradaron según diseño (FBO negro sin plano de fondo; textos negros invisibles sobre negro). **Sin defecto en la lib.** Fix solo en playground (`.jpg`→`.png` + comentario anti-divergencia) + minors :296/:300 de la review resueltos.
- **N3 ronda 2 (usuario, 2026-07-14): LOS 4 PUNTOS PASAN** (frente gold + textos, reactividad member/tier fallback sin recrear canvas, encaje de nombre largo/sRGB, 60 fps con drag).
- **Verificación final del leader**: build ✅ · lint ✅ · test 69/69 ✅.
- Feature 7 → `done`. **Fase 4 de spec-03 completa** (features 6+7; N3 de Fase 4 superada).
- **Nota del usuario (seguimiento)**: en el futuro habrá que diseñar el tema con assets mucho más potentes que los PNGs demo actuales (relevante para features 9 —temas de ejemplo— y 10 —documentación del tema—, y para assets de producción).
- **Housekeeping pendiente** (fuera de perímetro de features): extraer el stub canvas 2D duplicado a `setupFiles` compartido de vitest; normalizar CRLF de `badge.config.ts`; líneas >100 chars preexistentes en zonas committeadas.
- **Siguiente**: feature 8 `badge-theme-validation` (depende de 6 ✅) — pending.

## 2026-07-14/15 — spec-03 Fase 5 — feature 8 `badge-theme-validation` → done (review aprobada a la primera)

- **Rol**: leader (orquestación); 1 implementer → 1 reviewer (**APROBADO**, `progress/review_feature8.md`).
- **Implementer** (`progress/impl_feature8.md`): nueva fn pura `assertValidBadgeTheme(theme)` en `badge-theme.ts` — valida `defaultBaseTextureUrl` y `fontUrl` del tema RESUELTO (venga del input `[theme]` o del token), lanza UN solo `Error` con prefijo `[ngx-products-3d]` accionable que lista TODOS los campos faltantes (string vacío cuenta como faltante); integrada en el computed `resolvedTheme` de `badge.component.ts` → validación temprana en el primer CD, antes de recursos y de `beforeRender`; error previo de 'no theme' conservado byte a byte. NO exportada en `public-api.ts` (helper interno, como `badge-material.ts`/`badge-texture.ts`). Error duro SIEMPRE (sin `ngDevMode`: conventions — los errores de config duros van siempre; ngDevMode solo warnings). 7 tests nuevos (5 N1 en `badge-theme.spec.ts` + 2 de integración input/token en `badge.component.spec.ts`).
- **Review**: CA1–CA6 verificados; N2 íntegro re-ejecutado por el reviewer (build ✅ · lint ✅ · test **76/76** ✅ · playground build ✅); N3 N/A justificado (sin cambio visual con tema válido). 2 observaciones no bloqueantes: `REQUIRED_THEME_URL_FIELDS` sin prefijo `BADGE_` (renombrar si se vuelve a tocar el archivo) y `missing.join(' y ')` quedaría raro si algún día hay 3+ campos obligatorios.
- **Verificación final del leader (2026-07-15)**: build ✅ · lint ✅ · test 76/76 ✅.
- Feature 8 → `done`. **OJO**: features 6/7/8 siguen SIN commitear en el working tree (el usuario no ha pedido commit).
- **Siguiente**: feature 9 `playground-complete` (depende de 7 ✅). Recordar memoria: el usuario quiere en el futuro temas con assets mucho más potentes que los PNGs demo (relevante para el selector de temas de la 9).

## 2026-07-15 — spec-03 Fase 5 — feature 9 `playground-complete` → done (review aprobada a la primera; N3 usuario ✅)

- **Rol**: leader (orquestación); 1 implementer → 1 reviewer (**APROBADO**, `progress/review_feature9.md`) → N3 del usuario ✅ (2026-07-15).
- **Implementer** (`progress/impl_feature9.md`): SOLO playground, cero deps nuevas. Form con bindings nativos (sin ReactiveForms): name/memberNumber/tier → `member.update` inmutable; select de tier con `gold`/`silver`/`bronze` (bronze NO existe en `baseTextures` de ningún tema → fallback a `defaultBaseTextureUrl` verificable); typo inicial `'silverFUnc'` corregido a `'gold'`. Selector de temas: `DEMO_THEMES` (violet + **ember**) + `themeKey` signal + `theme = computed(...)`. Tema ember con 4 PNGs demo nuevos generados dep-free desde scratchpad (patrón `asset_playground_textures.md`, script NO versionado): `band-ember.png` 512×64 (gradiente periódico rojo→naranja, tilea con repeat [-4,1]) y `base-ember-{gold,silver,default}.png` 256×256, más `colors` band/clip/text propios → el cambio de tema se ve en correa/clip/texto, no solo en el frente. Controles FUERA del `@defer` (sin `<form>`: evita submit con Enter); nada re-monta el canvas. Toggle debug conservado. Gotcha `[theme]`-pisa-provider: tema violet idéntico al del provider + comentarios anti-divergencia en ambos archivos.
- **Review**: perímetro exacto, CA1–CA8 verificados, N2 íntegro re-ejecutado (build ✅ · lint ✅ · test **76/76** ✅ · playground ✅), PNGs binarios validados (firma+IHDR), sin deep imports. Menores no bloqueantes: `<select [value]>` sin CVA exige que el valor inicial sea la primera `<option>` (hoy se cumple); `height: 97vh` con espacios es preexistente. **Constató además que los cambios de lib de las features 6/7/8 ya están commiteados** (176ac42, 0fc5ac7, fb4a962) — `git diff` de la lib limpio.
- **N3 (usuario, 2026-07-15): OK** — reactividad del form sin recrear canvas, cambio de tema en vivo, fallback bronze, debug toggle y drag sin regresión.
- Feature 9 → `done`.
- **Siguiente**: feature 10 `readme-docs` (depende de 8 ✅). Recordar memoria: assets de producción de más calidad pendientes (el README documenta la tabla del tema — buen sitio para requisitos de assets).

## 2026-07-15 — spec-03 Fase 5 — feature 10 `readme-docs` → done (review aprobada a la primera, sin cambios requeridos)

- **Rol**: leader (orquestación); 1 implementer → 1 reviewer (**APROBADO**, `progress/review_feature10.md`). N3 N/A (solo documentación).
- **Implementer** (`progress/impl_feature10.md`): `projects/ngx-products-3d/README.md` reescrito por completo (era stub de 3 líneas; 332 líneas, empaquetado por ng-packagr en dist — verificado byte a byte). Contenido: instalación + tabla de los 9 peers literales del package.json (incluida ngxtension), nota de peso Rapier WASM/Three + contrato de consumo @defer en 3 pasos, quickstart compilable calcado del playground (providers EnvironmentProviders en ruta lazy: provideNgtRenderer + provideProducts3d + provideProducts3dBadgeTheme; componente con @defer (on viewport) + @placeholder), tabla COMPLETA de Products3dBadgeTheme campo a campo (obligatorios defaultBaseTextureUrl/fontUrl por la validación de la feature 8; gotcha [theme]-PISA-provider con la cadena input ?? token ?? Error), contrato GLB (nodos card/clip/clamp, materiales base/metal, dims 1.6×2.25×0.02, origen = anclaje del clip [0,1.45,0], advertencia collider fijo, Draco: gltfResource useDraco=true por defecto + setDecoderPath, verificado en soba), requisitos de assets del consumidor, sección avanzada Products3dBadgeScene (debug documentado honesto como "reservado") y nota de scheduler de CD (zoneless o zone.js). Idioma: español por homogeneidad del repo (decisión reversible en feature 11).
- **Review**: verificación INDEPENDIENTE fuerte — extrajo los 4 snippets TS del README y los compiló con tsc strict como consumidor externo contra los d.ts de dist: compilan; 15/15 símbolos en el d.ts; muestreo amplio símbolo-a-símbolo todo OK; contrato GLB contrastado con spike + código; N2 íntegro re-ejecutado (build ✅ · lint ✅ · test **76/76** ✅ · playground ✅ · dist sin deps fantasma). Cambios requeridos: NINGUNO. Observación principal no bloqueante: decidir idioma (ES vs EN) explícitamente en la feature 11 antes de publicar en npm.
