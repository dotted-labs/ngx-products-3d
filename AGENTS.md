...
1. Ejecuta `pnpm build && pnpm ng lint ngx-products-3d && pnpm ng test ngx-products-3d`
   y verifica que termina sin errores. Si falla, **para** y resuelve el entorno
...
| `projects/products-3d-playground/` | App demo consumidora de la lib                      | Para verificar integración |
| `**/*.spec.ts`               | Tests unitarios Vitest (junto al fuente)                  | Para verificar |
...
- **No declares una tarea `done` sin pruebas verdes.** Ejecuta
  ...
  y asegúrate de que todo pasa al 100%.
...
1. Ejecuta `pnpm build && pnpm ng lint ngx-products-3d && pnpm ng test ngx-products-3d` — todo verde.
2. Si la tarea está acabada: marca `status: "done"` en `feature_list.json`.
3. Mueve el resumen de `progress/current.md` al final de `progress/history.md`.
4. Vacía `progress/current.md` dejando solo la plantilla.
5. No dejes archivos temporales, ni `console.log` de debug en la lib, ni TODOs sin contexto.
